...
Do NOT report style, formatting, naming, or architectural preferences, and do NOT report a bug you could not confirm by reading the code — those will be discarded too.`,
  contextGuidance: `A diff hides the control flow around the lines it shows, so you MUST read the code BEFORE reporting a defect: use get_file to read the whole changed function and the guards, early returns, and loops surrounding it, get_base_file to see what the previous version did so you can tell a deliberate change from a mistake, and find_references on the changed function — with its name for every caller, without one for the files importing it — to confirm how they use its return value, its error, or its contract, get_symbol first when you need to know which definition the diff means and whether it is exported. An empty result proves nothing until index.languages says that language was indexed; when it was not, fall back to search_repository. If you did not read the code, do not report it.`,
};
