...
Do NOT report micro-optimisations, allocation counts, style, or architectural preference. Do NOT report code that runs once at startup, in a build script, or over an input whose size is fixed and small.`,
  contextGuidance: `Cost lives outside the diff, in how often the changed code runs and in what the call inside the loop does. Before reporting you MUST use find_references on the changed function — with its name for every caller, without one for the files importing it — to learn whether it sits on a hot path, called per request or once per row by another loop, get_symbol when you need to confirm which definition the diff means, and get_file to read the whole loop and the function it calls, since a diff hunk hides both the loop bound and the callee's body. Use search_repository to check whether a batched or paginated variant of the same query already exists, because that is the fix to suggest, and get_base_file when you need to show the loop is new rather than pre-existing. An empty reference result proves nothing until index.languages says that language was indexed. If you did not read the loop and the callee, do not report it.`,
};
