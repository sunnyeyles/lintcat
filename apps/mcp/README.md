...
|---|---|---|
| `review_local_changes` | Runs the general reviewer on commits since the merge-base with the base branch, plus uncommitted and untracked files. Returns only validated findings. | A model key, or a sampling client |
...
| `suppress_finding` | Marks a finding as a false positive, so later reviews of the same checkout exclude it. Matched by category and title shape. | Nothing |
...
There is no agent configuration. A checkout that still holds v2's
`.github/pr-review-agents.yml`, or a review call that passes the removed
