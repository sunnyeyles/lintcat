...
function openScoped(environment: McpEnvironment, args: ScopeArgs): Promise<LocalRepository> {
  return openLocalRepository(
    path.resolve(environment.cwd, args.repoPath ?? "."),
    args.base,
    chooseScope(args),
  );
}
...
export function registerReviewTools(server: McpServer, environment: McpEnvironment): void {
  server.registerTool(
    ...
    async (args) => {
      const local = await openScoped(environment, args);
